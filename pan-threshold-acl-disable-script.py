# Script name: pan-threshold-acl-disable-script.py
# Purpose - This script will disable Threshold ACL for limiting apple updates.
...
# API calls for checking stale configs, disabling Threshold ACL on the interface and Commit


check_api = (('type', 'op'),('cmd', '<check><pending-changes></pending-changes></check>'),('key', sys.argv[1]),)
acl_api = (('key', sys.argv[1]),('type', 'config'),('action', 'set'),('xpath', '/config/devices/entry[@name=\'localhost.localdomain\']/vsys/entry[@name=\'vsys1\']/rulebase/security/rules/entry[@name=\'Threshold-Updates\']'),('element', '<disabled>yes</disabled>'),)
...
if status == 'success':
    ...
    print(status)
    if status == 'success':
        print('Commit successful! Threshold ACL has been disabled successfully.')
    else:
