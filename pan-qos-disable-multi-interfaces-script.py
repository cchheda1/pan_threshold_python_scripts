# Script name: pan-qos-disable-multi-interfaces-script.py
# Purpose - This script will disable qos on multiple PAN interfaces for limiting apple updates.
...
# API calls for checking stale configs, disabling QoS on the interface and Commit

check_api = (('type', 'op'),('cmd', '<check><pending-changes></pending-changes></check>'),('key', sys.argv[1]),)
int1_api = (('key', sys.argv[1]),('type', 'config'),('action', 'set'),('xpath','/config/devices/entry[@name=\'localhost.localdomain\']/network/qos/interface/entry[@name=\'ethernet1/1\']'),('element', '<enabled>no</enabled>'),)
int2_api = (('key', sys.argv[1]),('type', 'config'),('action', 'set'),('xpath','/config/devices/entry[@name=\'localhost.localdomain\']/network/qos/interface/entry[@name=\'ethernet1/3\']'),('element', '<enabled>no</enabled>'),)
int3_api = (('key', sys.argv[1]),('type', 'config'),('action', 'set'),('xpath','/config/devices/entry[@name=\'localhost.localdomain\']/network/qos/interface/entry[@name=\'ethernet1/5\']'),('element', '<enabled>no</enabled>'),)
int4_api = (('key', sys.argv[1]),('type', 'config'),('action', 'set'),('xpath','/config/devices/entry[@name=\'localhost.localdomain\']/network/qos/interface/entry[@name=\'ethernet1/7\']'),('element', '<enabled>no</enabled>'),)
    ...
    if status == 'success':
        ...
        if status == 'success':
            ...
            if status == 'success':
                ...
                print(status)
                if status == 'success':
                    print('Commit successful! QoS has been disabled successfully.')
                else:
